...
The input antigen-antibody **complex** (AAC) is represented by a list C = [v1, v2, …, vn] of n **contacts**. Each j-th contact is a vector vj = [xj0, xj1, …, xjm] of mj elements, where xj0 is the antibody residue interacting with the xj1, …, xjm residues on the surface of the receptor binding domain (RBD) of the Spike protein variant. Every contact corresponds univocally to a vector ai = affinity(vj) of 101 affinity score values, ranging from 0 to 1, and corresponding to the 101 nanoseconds of the molecular dynamics simulation. Computationally, the AAC is specified as follows (R code):
